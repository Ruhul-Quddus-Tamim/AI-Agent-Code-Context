...
use vers::{utils, HNSWIndex, Vector};
...
fn test_wiki_ivfflat(
    ...
    let mut vectors: Vec<Vector<300>> = vectors.iter().map(|v| v.0).collect();

    // make mutable copies to satisfy utils::test_ivfflat signature
    let mut word_to_idx = word_to_idx;
